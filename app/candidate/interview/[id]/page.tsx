...
export default function CandidateInterviewPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            client.on?.("error", (error: Error) => {
                // Only handle real errors, ignore empty objects
    ...
    const startInterview = async () => {
        ...
        try {
            const assistantId = "3006f3f1-d975-4d0b-b595-bb9141beea1d";
            
            if (!assistantId) {
                setErrorMessage("Voice assistant not configured");
                setStatus("error");
                return;
            }
            ...
            // Create custom assistant config with YOUR generated questions
            const customAssistant = {
                model: {
                    ...
                    messages: [
                        {
                            role: "system",
                            content: `You are a professional technical interviewer conducting a ${interview.type} interview for a ${interview.level} ${interview.role} position. 

Tech Stack Focus: ${interview.techstack.join(", ")}
...
- After all questions are answered, thank the candidate and end the interview`
            ...
            const result = await vapiClient.start(customAssistant);
